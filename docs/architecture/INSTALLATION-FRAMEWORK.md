...
- `canary_admission_oracle.py --self-test` / `--target <canary>` — maintainer/canary-admission gate. BLOCKS readiness when a candidate target is not a Git work tree, lacks the Field Reports `pre-push` gate, or lacks strict pre-commit proof, and never emits `git_clean` / `prepush_installed` / `precommit_enforced` without material Git evidence. It also classifies each host's native hook claim from that host's OWN runtime records: a configured-but-unobserved host is `CONFIGURED_NOT_OBSERVED` (never native PASS), and records from one host never fill another host's claim. Strict pre-commit here is **canary admission infrastructure, not TES default adopter behavior** — TES does not auto-install a strict pre-commit gate for adopters (project-scoped hooks stay idempotent and advisory).
