...
PreToolUse has a deliberately split internal contract. `scripts/pretooluse_kernel.py` owns the host-neutral decision kernel: event/tool normalization, patch/path extraction, governed-surface detection, forbidden classification input, and the resulting allow/supervise/block record. `scripts/pretooluse_session.py` owns the session coordinator: the anti-cry-wolf sentinel and same-session suppression for governed supervision. `tes_install.py` owns integration: Cortex advisory context, runtime ledger writes, hook installation, and host-specific renderers. Do not move Claude `hookSpecificOutput`, Codex stderr text, Cursor `permission` JSON, installer/update logic, Field Reports, sync, release, postinstall advisories, or ledger writes into either PreToolUse helper.
