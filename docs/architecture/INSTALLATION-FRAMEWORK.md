...
- `canary_admission_oracle.py --self-test` / `--target <canary>` — maintainer/canary-admission gate. BLOCKS readiness when a candidate target is not a Git work tree, lacks the Field Reports `pre-push` drain, or lacks strict pre-commit proof, and never emits `git_clean` / `prepush_installed` / `precommit_enforced` without material Git evidence. It also classifies each host's native hook claim from that host's OWN runtime records: a configured-but-unobserved host is `CONFIGURED_NOT_OBSERVED` (never native PASS), and records from one host never fill another host's claim. On Git-eligible targets TES installs and verifies a strict pre-commit wrapper: it runs project `commit:check` when present, then a materialized TES discipline oracle, and otherwise fails closed. The Field Reports `pre-push` remains an advisory drain; a separate project pre-push gate is reported observationally, not required.
