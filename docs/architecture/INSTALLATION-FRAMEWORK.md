...
| git | active `pre-push` hook (`core.hooksPath` aware) | strict pre-push quality gate plus Field Reports drain |
...
PreToolUse's canonical behavioral contract is `docs/architecture/PRETOOLUSE-CONTRACT.md`. The implementation stays deliberately split: `scripts/pretooluse_kernel.py` owns the host-neutral decision kernel, `scripts/pretooluse_session.py` owns only anti-cry-wolf repetition state, and `tes_install.py` owns Cortex advisory context, runtime ledger writes, hook installation, and host-specific renderers. Do not move Claude `hookSpecificOutput`, Codex stderr text, Cursor `permission` JSON, installer/update logic, Field Reports, sync, release, postinstall advisories, or ledger writes into either PreToolUse helper.

PreToolUse observability should read the runtime ledger as host evidence, not as a normalized UI event stream. Cursor can label a native StrReplace UI action as `tool: "Write"` before TES sees the payload; that is host payload labeling, not a kernel defect, when the ledger still shows the governed path with `risk=material`, `marker_emitted=true`, and `context_repeated=true` plus `anti_crywolf_repeated_context` when applicable. External dedup and analytics must not key only on `invocation` plus timestamp; include `tool`, `risk`, `path` or `command`, and `session`/`mode` when present so batched Cursor projections and parallel host projections are not misreported as duplicate hook execution.
...
- `git_gate_contract.py --self-test` — canonical Git gate contract. Owns the evidence rules, standardized fixtures, and quantitative matrix for no-Git, missing hooks, drain-only, quality-only, unresolved-gate, echo-token, full, and dirty states.
- `canary_admission_oracle.py --self-test` / `--target <canary>` — maintainer/canary-admission consumer of `git_gate_contract.py`. BLOCKS readiness when a candidate target is not a Git work tree, lacks the Field Reports `pre-push` drain, lacks strict pre-push proof, lacks strict pre-commit proof, or is dirty, and never emits `git_clean` / `prepush_installed` / `prepush_enforced` / `precommit_enforced` without material Git evidence. It also classifies each host's native hook claim from that host's OWN runtime records: a configured-but-unobserved host is `CONFIGURED_NOT_OBSERVED` (never native PASS), and records from one host never fill another host's claim. On Git-eligible targets TES installs and verifies strict pre-commit and pre-push wrappers: project `prepush:check` or `commit:check` wins when present, then a materialized TES discipline oracle, and otherwise the hook fails closed. The Field Reports `pre-push` drain remains advisory and additive; it never satisfies the pre-push quality gate.
- `canary_transcript_oracle.py --target <canary> --latest --include-subagents --require-tool-use --json-only` — sanitized Claude Code transcript sidecar for host-real canaries. It proves a local transcript exists, parses JSONL, counts user/assistant/tool/subagent evidence, and emits hashes only; raw messages, tool inputs, tool results, and subagent metadata stay out of the report. This sidecar strengthens canary replay evidence but does not replace Git admission.
- `git_gate_convergence_loop.py` / `npm run git-gate:converge` — short recursive loop for Git gate changes: contract matrix, canary admission, installed certification, Field Reports, hook-manager awareness, and installer self-test.

A passing source self-test is not enough when the risk lives in an installed, materialized, or host-real surface; the canary battery in `~/Dev/tes-canaries` exercises real and synthetic target projects, and the host-real canary (open the project in the actual host and observe the hook fire) is the only thing that upgrades a claim from "hooks written correctly to disk and bench-certified" to "hooks converged in real use". A canary may only claim `READY_FOR_GOAL_MAESTRO_CANARY` after `canary_admission_oracle.py` passes against it.
