...
| Cursor | `preToolUse` uses the host JSON permission contract. Forbidden actions return JSON permission deny with process exit `0`; governed material edits are allowed and may include JSON `user_message`, but native allow-path visibility can be ledger-only; the runtime ledger/sentinel is the canonical proof for first governed allow supervision. Benign work is silent. |
