...
| Codex | `SessionStart` and `PreToolUse` are installed through TOML hook blocks. Forbidden actions block with exit `2` plus `stderr`; governed material edits are supervised; benign work is silent. |
