...
Version `0.3.193` is the fixed release identity for this install surface; treat the remote ref as certified only after the tag is published and `npm run release:check` passes.
...
`#v0.3.193` is the intended fixed-ref form for a release-certified install
