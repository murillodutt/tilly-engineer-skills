...
The public installer path is the GitHub package-spec command through npx or Bun after a fixed ref is authorized and release-certified. Version `0.3.234` is the fixed release identity for this install surface; treat the remote ref as certified only after the tag is published and `npm run release:check` passes.
...
```bash
npx --loglevel=error -y --package github:murillodutt/tilly-engineer-skills#v0.3.234 tilly-engineer-skills add
```

```bash
bunx --silent --bun --package github:murillodutt/tilly-engineer-skills#v0.3.234 tilly-engineer-skills add
```
...
```bash
npx --loglevel=error -y --package github:murillodutt/tilly-engineer-skills#v0.3.234 tilly-engineer-skills add --agent all --yes
```

`#v0.3.234` is the intended fixed-ref form for a release-certified install channel. Do not call that remote ref certified until `npm run release:check` passes after the tag or fixed ref is authorized and available.
