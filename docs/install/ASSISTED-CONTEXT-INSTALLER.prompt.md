...
If legacy retirement is required, run `python3 scripts/tes_legacy_retirement.py plan --target <target-root>` after the root context gate and before materializing new assets. The gate may remove only known old runtime assets, migrate `.tilly/field-reports/**` to `.tes/field-reports/**`, archive `.tilly/retrofit/**` and legacy hook ledgers under `.tes/legacy-retirement/**`, compact exact duplicate runtime hook rows with an archive copy, remove ephemeral hook-smoke harness scripts, and preserve project context plus `SKILL.md` smoke evidence. If it reports `NEEDS_REVIEW`, stop; do not copy new TES assets over old surfaces. When it is clean and the run is authorized, apply with `python3 scripts/tes_legacy_retirement.py apply --target <target-root> --yes`, then certify with `python3 scripts/tes_legacy_retirement.py audit --target <target-root>`.
