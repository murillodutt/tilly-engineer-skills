...
navigation_renderer: anthropic-api navigation_mode_preference: custom-tool-use

## Platform Reading

Direct Anthropic API usage has no built-in installer UI. A client harness must render navigation intents and return selected commands.

## Renderer Order

1. Use tool-use or application protocol fields to request a labeled menu from the client harness.
