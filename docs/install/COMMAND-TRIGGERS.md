...
| `/tes-mcp` or `/tes:mcp` | activate or verify Cortex MCP | `install_mcp.py`, `cortex_mcp.py`, MCP smoke | `.tes/bin/**` and project-scoped MCP config; governed remember is default; use `--read-only` for inspection-only activation |
...
| `npx --loglevel=error -y --package github:murillodutt/tilly-engineer-skills#v0.3.140 tilly-engineer-skills add` | fixed GitHub npx installer entrypoint |
