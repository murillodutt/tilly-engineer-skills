...
| `/tes-goal-maestro` or `/tes:goal-maestro` | materialize a mature SPEC, Super SPEC, PRD, relational project plan, or accepted execution tree into an execution-grade tree and a ready maestral `/goal` prompt when internal gates pass; optional `next_prompt_handoff=true` or `--next-prompt-handoff` adds chat-only next-prompt emission after certification; optional `--execute-loop` runs the parent-controlled active-SPEC execution loop | active agent artifact review; `NEEDS_SPEC_MATURITY`, `NEEDS_EXECUTION_UNIT_FIDELITY`, `NEEDS_TREE_REPAIR`, `DRAFT_MATERIALIZATION_TREE`, `NEEDS_TREE_ACCEPTANCE`, `SUPER_SPEC_MATERIALIZED`, `READY_GOAL_PROMPT`, `NEEDS_EXECUTION_LOOP_DRAFT`, `SPEC_BLOCKED`, `SPEC_CONTRACT_UNSTABLE`, `EXECUTION_LOOP_COMPLETE`, or `EXECUTION_LOOP_COMPLETE_WITH_AUDIT_REPAIRS` status | generated Super SPECs are written to `GOAL-SUPER-SPEC-*.md`; prompt/tree output stays chat-first unless explicitly saved; next prompt handoff never writes or executes automatically; `--execute-loop` may create local commits per green SPEC but never pushes without separate authorization |
...
| `npx --loglevel=error -y --package github:murillodutt/tilly-engineer-skills#v0.3.183 tilly-engineer-skills add` | fixed GitHub npx installer entrypoint |
...
- Do not give `/tes-prospect`, `/tes-mine`, or `/tes-goal-maestro` broad
  ...
  Executive Stop Audit before final closure.
