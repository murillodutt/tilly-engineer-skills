...
| `/tes-goal-maestro` or `/tes:goal-maestro` | materialize a mature SPEC, Super SPEC, PRD, relational project plan, or accepted execution tree into an execution-grade tree and a ready maestral `/goal` prompt when internal gates pass; optional `next_prompt_handoff=true` or `--next-prompt-handoff` adds chat-only next-prompt emission after certification | active agent artifact review; `NEEDS_SPEC_MATURITY`, `NEEDS_EXECUTION_UNIT_FIDELITY`, `NEEDS_TREE_REPAIR`, `DRAFT_MATERIALIZATION_TREE`, `NEEDS_TREE_ACCEPTANCE`, `SUPER_SPEC_MATERIALIZED`, or `READY_GOAL_PROMPT` status | generated Super SPECs are written to `GOAL-SUPER-SPEC-*.md`; prompt/tree output stays chat-first unless explicitly saved; next prompt handoff never writes or executes automatically |
