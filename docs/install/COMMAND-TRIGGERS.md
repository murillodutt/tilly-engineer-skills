...
`/tes-prospect`, `/tes-mine`, and `/tes-goal-maestro` are explicit-invocation skills. They do not have broad natural-language routing. Prospecting and mining stay dormant until the user names the skill or trigger, then operate proactively with a cognitive brake. Goal maestro may also route from a direct request to generate a maestral `/goal` prompt from a mature SPEC, Super SPEC, PRD, relational project plan, or accepted execution tree. `--execute-loop` is the Goal Maestro execution parameter. `--audit-heartbeat-prompt` generates the Universal Adversarial Audit Heartbeat prompt: standalone use emits only the copy-ready prompt, while `/tes-goal-maestro --execute-loop --audit-heartbeat-prompt` emits it as a same-response read-only sidecar, not a second Goal Maestro command. Structured opt-ins are `audit_heartbeat=true` and `adversarial_audit_heartbeat: requested`, and a direct request to generate or create an adversarial audit heartbeat prompt is also valid. Broad words such as audit, monitor, mentorship, or heartbeat do not activate it. For code, UI, canvas, or 3D work, Goal Maestro also records the structural method and requires browser metrics or visual-spatial evidence when the user-facing result depends on pixels. `/tes-bump` is the version governance guard: it routes from direct bump/sync requests and auto-activates read-only when commit, release, delivered behavior, or another TES gate reports a version-decision condition. The executable parity gate is `python3 scripts/command_trigger_oracle.py --self-test`. Installed target parity can be checked with `python3 scripts/command_trigger_oracle.py --target <target-root>`.
...
| `/tes-goal-maestro` or `/tes:goal-maestro` | materialize a mature SPEC, Super SPEC, PRD, relational project plan, or accepted execution tree into an execution-grade tree and a ready maestral `/goal` prompt when internal gates pass; optional `next_prompt_handoff=true` or `--next-prompt-handoff` adds chat-only next-prompt emission after certification; optional `--execute-loop` runs the parent-controlled active-SPEC execution loop, treats reference implementations as baseline-only comparison, and owns internal continuation when both options are requested; optional `--execute-loop-parent-fallback` allows parent-side worker fallback only when worker capacity is unavailable; optional `--audit-heartbeat-prompt`, `audit_heartbeat=true`, `adversarial_audit_heartbeat: requested`, or a direct request to generate/create an adversarial audit heartbeat prompt emits the Universal Adversarial Audit Heartbeat prompt as standalone output or, with `--execute-loop`, as a same-response read-only sidecar | active agent artifact review; structural method gate for code work; browser metrics or visual-spatial oracle for UI/canvas/3D work; heartbeat prompt contract check; `NEEDS_SPEC_MATURITY`, `NEEDS_EXECUTION_UNIT_FIDELITY`, `NEEDS_TREE_REPAIR`, `DRAFT_MATERIALIZATION_TREE`, `NEEDS_TREE_ACCEPTANCE`, `SUPER_SPEC_MATERIALIZED`, `READY_GOAL_PROMPT`, `NEEDS_EXECUTION_LOOP_DRAFT`, `SPEC_BLOCKED`, `SPEC_CONTRACT_UNSTABLE`, `NEEDS_MORE_LOOPS`, `NEEDS_OWNER_DECISION`, `SAFETY_BLOCKED`, `EXECUTION_LOOP_COMPLETE`, or `EXECUTION_LOOP_COMPLETE_WITH_AUDIT_REPAIRS` status; heartbeat prompt status vocabulary remains separate and may include `HEARTBEAT_BLOCKED_CONTEXT` inside the emitted prompt | generated Super SPECs are written to `GOAL-SUPER-SPEC-*.md`; prompt/tree output stays chat-first unless explicitly saved; next prompt handoff never writes or executes automatically outside the parent-owned execution loop; `--audit-heartbeat-prompt` never edits, stages, commits, pushes, calls remotes, schedules host jobs, shares data, owns Goal Maestro stop states, or creates a second Goal Maestro command; `--execute-loop` may create local commits per green SPEC, failed-attempt recovery evidence, structural/browser/visual evidence, audit repairs, and `GOAL-EXECUTION-LOOP-LEDGER-*.md` for long/repaired loops, but never pushes, never treats manual/reference builds as execution credit, or performs cloud escalation without separate authorization |
...
generate/create an adversarial audit heartbeat prompt -> /tes-goal-maestro --audit-heartbeat-prompt
...
| `npx --loglevel=error -y --package github:murillodutt/tilly-engineer-skills#v0.3.252 tilly-engineer-skills add` | fixed GitHub npx installer entrypoint |
...
| Hook health | installed-target hook evidence: `tes_install.py hook-health --target . --json-only --agent <current-host>` scopes ceiling evidence to the host under audit; omit `--agent` only for the all-configured-host aggregate. It reports each Claude Code, Codex, and Cursor lifecycle event as `OBSERVED`, `CONFIGURED`, `STALE/UNEXPECTED`, or `NOT_CONFIGURED` from current runtime ledgers and host config; `status` remains the legacy functional field (`PASS`, `PASS_WITH_FINDINGS`, `NEEDS_EVIDENCE`, `DEGRADED`), while `floor_status`, `ceiling_status`, and `ceiling_gaps` separate `PASS_BASIC` from `PASS_CEILING` evidence |
