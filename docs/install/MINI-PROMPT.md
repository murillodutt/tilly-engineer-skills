...
Across Codex, Claude Code, and Cursor, prefer shared hyphen triggers such as `/tes-init`, `/tes-setup`, `/tes-update`, `/tes-goal-maestro`, `/tes-prospect`, `/tes-mine`, and `/tes-cortex`. If a host reports a `/tes:*` alias as invalid, treat it as TES intent text and continue through the matching skill/rule/spec. `/tes-setup` is the direct setup alias for `/tes-init`; `/tes-goal-maestro`, `/tes-prospect`, and `/tes-mine` require explicit invocation, and `tes-goal-maestro` may also route from a direct request to generate a maestral `/goal` prompt from a mature ADR, PRD, SPEC, Super SPEC, relational project plan, accepted execution tree, or contextualized objective. Goal Maestro owns prompt enrichment: it derives the execution tree, active-SPEC queue, structural method, oracle packet, evidence contract, stop states, local report sidecars, and optional prompt sidecars before emitting `/goal` after internal tree, material-diff, material-continuation, semantic negative-grep, sequential ownership, and sync-status gates pass. `--execute-loop` is the Goal Maestro execution parameter. `--audit-heartbeat-prompt`, `audit_heartbeat=true`, `adversarial_audit_heartbeat: requested`, or a direct request to generate/create an adversarial audit heartbeat prompt emits the copy-ready English read-only Heartbeat prompt standalone, or as a same-response read-only sidecar when combined with `--execute-loop`; it never edits, schedules, shares, owns Goal Maestro stop states, or creates a second Goal Maestro command. Generated Super SPEC content is written to `GOAL-SUPER-SPEC-*.md` and summarized in chat instead of being pasted into the context window.
