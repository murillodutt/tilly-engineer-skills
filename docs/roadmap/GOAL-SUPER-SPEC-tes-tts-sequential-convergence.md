...
Certified evidence from prior cycle:
- TTS-027 re-read ADR 0004, all previous TES TTS owner decision records from
  TTS-010 onward, the TTS-009 decision record, the TES TTS roadmap, this Super
  SPEC, and the TTS-027 prompt.
- TTS-027 found no explicit maintainer decision in the current goal context to
...
- TTS-027 recorded the owner decision still remains unresolved again result at
  docs/roadmap/TES-TTS-OWNER-DECISION-STILL-REMAINS-UNRESOLVED-AGAIN.md.
- TTS-027 updated `docs/roadmap/TES-TTS-SKILL-ROADMAP.md` with the cycle
...
- TTS-027 made no provider certification claim and performed no sync, release,
...
- Ready prompt artifact:
  docs/roadmap/GOAL-PROMPT-tes-tts-TTS-028-owner-decision-remains-open-again.md.
- TTS-027 focused oracles passed:
...
Task:
Execute only TTS-028 through the circular sequence:
execute -> analyze -> fix -> certify -> create next /goal prompt -> local commit.
