...
Certified evidence from prior cycle:
- TTS-019 re-read ADR 0004, all previous TES TTS owner decision records from
  TTS-010 onward, the TTS-009 decision record, the TES TTS roadmap, this Super
  SPEC, and the TTS-019 prompt.
- TTS-019 found no explicit maintainer decision in the current goal context to
...
- TTS-019 recorded the owner decision still unresolved result at
  docs/roadmap/TES-TTS-OWNER-DECISION-STILL-UNRESOLVED.md.
- TTS-019 updated `docs/roadmap/TES-TTS-SKILL-ROADMAP.md` with the cycle
...
- TTS-019 made no provider certification claim and performed no sync, release,
...
- Ready prompt artifact:
  docs/roadmap/GOAL-PROMPT-tes-tts-TTS-020-owner-decision-continues-unresolved.md.
- TTS-019 focused oracles passed:
...
Task:
Execute only TTS-020 through the circular sequence:
execute -> analyze -> fix -> certify -> create next /goal prompt -> local commit.
