...
| `scripts/tes_tts_omnivoice_provider.py` | Optional OmniVoice status, warm-cache, resident session, live-smoke, auto latency-profile selection from sealed profile reviews, product-status cockpit, candidate replay/open cockpit, speak, bench, review, decide-review, package-review, probe, synthesize, normalize-ref, and batch facade. | active |
