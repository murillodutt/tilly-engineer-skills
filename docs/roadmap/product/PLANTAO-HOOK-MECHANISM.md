...
- **`exit 2` bloqueia, stderr vira feedback ao agente.** `block_memory_write.sh:10-12` — *"0 = allow the tool call; 2 = block the tool call (stderr is shown to Claude as feedback)"*. Entrada por stdin JSON (`tool_name`, `tool_input`); guarda decide bloqueio por pattern-match no path.
- **`updatedInput` reescreve os parâmetros mid-flight.** `enforce_metadata_defaults.sh:197-204` emite `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow","updatedInput":$updated}}` — a chamada do agente continua, mas com os parâmetros substituídos pelo hook.
- **`additionalContext` injeta contexto.** `enforce_metadata_defaults.sh:13-16` declara o contrato; o campo `additionalContext` está disponível (não usado naquele script) para injetar ADR/SPEC/constraint antes da execução do tool.
- **Matcher economiza.** O campo `matcher` é o plano de controle: regex OU pipe de nomes de tool (`Write|Edit|MultiEdit`, `Read`, `Bash`) — `codex-hooks.json:15`, `cursor-hooks.json:17`. O matcher é **vocabulário transferível**; o comando ligado a ele é **host-privado**.
...
- `on_user_prompt.sh:4-7` (header verbatim): *"Instead of pre-searching … this injects a decision rubric telling the agent when and how to search itself. The agent has more context than this script does — let it decide."*
- O hook detecta sinais e **emite as detecções como contexto** (`on_user_prompt.sh:99-112`, `:159-170`); **nunca dispara a ação ele mesmo**. A rubrica é uma frase única por sessão (`:160`).
...
- **Intenção por regex** — `on_user_prompt.sh:65-71` (HAS_ERROR: `Traceback|panic:`, `fatal:`, 2+ keywords de erro), `:74` (FILE_PATHS por extensão), `:78-80` (HAS_RESUME), `:84-86` (HAS_REMEMBER). Sem API, sem DB.
- **Anti-cry-wolf por camadas** — gate de comprimento (`:24-26`, sai se prompt < 20 chars); `RUBRIC_FLAG` dedup por cache de sessão (`:42-47` lê, `:159-162` escreve — rubrica completa **uma vez por sessão**); nudge periódico por contagem de mensagem (`:49-61`, a cada 5ª) e captura sobreposta (`:178-180`, a cada 3ª).
...
### Mecanismo D — Instalador idempotente multi-host (`OWNER_MARKER` strip-then-merge)

- `install_codex_hooks.py:43` `OWNER_MARKER = "<ref-plugin>"` (nome de diretório estável, **não** nome de produto); `:62-76` `is_owned_entry()` + `strip_owned_entries()`; `:79-83` `merge_template()`. Em cada install: tira as entradas cujo comando contém o marker, depois mescla o template fresco — re-run (upgrade) não duplica.
- Gate por feature-flag (`:91-110`, `codex_hooks=true` em `~/.codex/config.toml`); guarda Windows `.sh` (`:131-140`).
- Manifesto por host declara o path de hooks (`codex-plugin/plugin.json:16` `"hooks": "./hooks/codex-hooks.json"`); o instalador reescreve esse JSON e o grava no config-dir do agente.

**Mapeamento ao gerente sênior.** É a folha de plantão idempotente: o gerente entra na escala uma vez por host (Claude `.claude/settings.json`, Cursor `.cursor/hooks.json` camelCase, Codex `.codex/config.toml` array-of-tables), e reinstalar não cria gerentes duplicados. TES já tem o marker (`.tes/bin/tes_install.py` usado em `tes_install.py:319`, `:350` para detecção) e já tem `ensure_hook_group` (`:467`) — o instalador vira um **materializador burro**: lê manifesto, reescreve paths, mescla, verifica flag. O Mantra-Gate **possui** a definição de evento/matcher; o instalador não.
...
| `smallest_patch` | Generalizar `ensure_hook_group` para `PreToolUse`/`UserPromptSubmit`; generalizar strip/merge para casar qualquer evento TES por `OWNER_MARKER`; materializar 2 scripts de hook (pré-ação, intenção) com vocabulário de matcher TES-namespace. |
...
ADR 0005 invariante 3 (`:96`) + `<confidentiality>` do bootloader. **Atravessa (forma):** o triplo `evento → matcher → timeout`; o schema de saída (`exit 0/2`, `hookSpecificOutput.updatedInput`, `additionalContext`); a rubrica-que-não-decide; os regex-shapes de intenção (erro/file-path/resume/save); o dedup por cache de sessão; o `OWNER_MARKER` strip-then-merge; o gate por feature-flag e a guarda Windows `.sh`. `statusMessage` é feature opcional só-Codex (`codex-hooks.json:42,53,98`) — atravessa como conhecimento de capacidade, não como string.

**NÃO atravessa (domínio/identidade o projeto-referência):**

1. **Regex de tool MCP do o projeto-referência** — `<ref-mcp-tool>`, `search_memories`, `get_memories`, `delete_all_memories`, namespace `<ref-mcp-namespace>` (`codex-hooks.json:15`, `cursor-hooks.json:17`). Os matchers de TES usam **TES-namespace**, não estes.
2. **Nomes de hook <ref-hook>** — `<ref-hook>`, `<ref-hook>`, `<ref-hook>`, `<ref-hook>`, `<ref-hook>`, `<ref-hook>`, `<ref-hook>`, `<ref-hook>`, `<ref-hook>` (`hooks.json:8,14,25,38,48,59,71,84,95`).
3. **Paths de comando e env vars <ref-hook>** — `${extensionPath}/scripts/`, `${PLUGIN_ROOT}/scripts/`, `${CURSOR_PLUGIN_ROOT}/scripts/`, `ANTIGRAVITY_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `<REF_ENV>`, `<REF_ENV>`, `<REF_ENV>`, `<REF_ENV>`; scripts `block_memory_write.sh`, `on_session_start.sh`, `on_user_prompt.sh`, `on_post_tool_use.sh`, `on_bash_output.sh`, `on_pre_compact.sh`, `_search.py`, `_identity.sh`, `telemetry.py`, `auto_capture.py`.
4. **Schema de identidade/memória o projeto-referência** — `user_id`, `app_id`/`project_id`/`<REF_ENV>`, campos `confidence`/`files`/`source`/`type`/`session_id`, filtros `metadata_type` (`session_state`, `decision`, `anti_pattern`, `task_learning`), paths temp `<ref-temp-path>`. Conceitos de "memory", "personalization", "semantic-search".
5. **Metadata de plugin do projeto-referência** — nome/versão/autor/homepage/keywords dos `plugin.json` (`.codex-plugin`, `.claude-plugin`), bloco `userConfig` de API key.

Regra de fronteira: se uma string carrega o nome do projeto-referência, seu prefixo de env var, um nome de tool MCP, ou um conceito de domínio dele (memória/usuário/personalização), **fica fora** do conteúdo TES rastreado. Só o *shape* (códigos de saída, schema JSON, eventos, ordem de fluxo) cruza.
...
| 2 | **Keyword-triggers** — rubrica por intenção: regex de risco/dúvida + dedup `RUBRIC_FLAG`, emitindo contexto sem decidir (Mecanismos B+C). | 1 | `materialize_adapter.py` (script de intenção materializado) | Band-oracle `interrupt_rate` dentro do intervalo honesto; dedup prova 1 injeção/sessão e 2 em sessões distintas; gate de comprimento < 20 chars sai silencioso. | **Delivered** → `owner_deferred` (bump de patch). |
