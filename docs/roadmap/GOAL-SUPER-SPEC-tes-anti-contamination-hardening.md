...
`docs/roadmap/GOAL-SUPER-SPEC-tes-anti-contamination-hardening.md`
...
```text
Objective: Execute the TES Anti-Contamination Hardening Super SPEC.

Canonical source:
`docs/roadmap/GOAL-SUPER-SPEC-tes-anti-contamination-hardening.md`

Operate as TES maestro. Start with Stage Zero (`git status`, `git log -8`,
dirty-change classification, and explicit confirmation that private target
evidence stays outside TES). Execute SPEC-000 through SPEC-007 sequentially.
Before each unit, state owned files, no-touch files, release identity impact,
focused oracle, and stop condition. After each unit, run the focused oracle,
fix failures, update correlated docs/indexes only when required, and run the
private vocabulary oracle when fixtures, docs, reports, or public surfaces are
touched. Hard boundaries: no private identifiers, write-capable MCP,
automatic Cortex writes, external memory backend, external datasets, UI/runtime
expansion, push, publish, tag, marketplace, remote, cloud action, or sealed
claim without `npm run commit:check`. Final closure reports units, files,
oracles, release identity, residual risks, deferred work, private vocabulary
result, and `npm run commit:check` status.
```
