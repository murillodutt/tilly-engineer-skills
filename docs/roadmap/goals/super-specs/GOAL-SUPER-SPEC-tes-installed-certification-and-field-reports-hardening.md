...
```text
/goal Execute the TES Installed Certification And Field Reports Hardening Super
SPEC in docs/roadmap/goals/super-specs/GOAL-SUPER-SPEC-tes-installed-certification-and-field-reports-hardening.md.

Read ADR 0003, ADR 0003.1, issue #46, the maintainer-local audit summary if
available outside tracked TES docs, AGENTS.md, the maintainer correlation rule,
and platform differences before editing.

Do not push, publish, tag, release, rebuild public bundles, or commit private
target names/paths. Reproduce the partial-certification failure in a neutral
fixture first, then repair package source and prove the installed-target
contract. Treat MCP config success as necessary but insufficient for final
PASS. Improve Field Reports only within the privacy boundary.
```
