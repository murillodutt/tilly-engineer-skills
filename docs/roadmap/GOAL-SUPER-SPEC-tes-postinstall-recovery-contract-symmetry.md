...
Canonical artifact:
`docs/roadmap/GOAL-SUPER-SPEC-tes-postinstall-recovery-contract-symmetry.md`
...
Use this file as the canonical execution contract.

```text
/goal Execute the TES Postinstall Recovery Contract Symmetry Super SPEC in
docs/roadmap/GOAL-SUPER-SPEC-tes-postinstall-recovery-contract-symmetry.md.

Read AGENTS.md, issue #47, the maintainer-local private canary analysis if
available outside tracked TES docs, the maintainer correlation rule,
scripts/tes_init.py, scripts/project_context_oracle.py,
scripts/installed_certification_oracle.py, scripts/command_trigger_oracle.py,
and scripts/mantra_gate_adoption_oracle.py before editing.

Do not copy private canary names, paths, target docs, remotes, commits, or
product vocabulary into TES. Reproduce the ignored-lockfile failure in a
neutral fixture first, then repair package source so generator and oracle share
the same contract. Do not push, publish, tag, release, rebuild public bundles,
or claim remote package certification without explicit release authorization.
```
