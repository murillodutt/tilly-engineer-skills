...
- `docs/roadmap/GOAL-SUPER-SPEC-cortex-hardening.md` only for status update.
- TDS/index files only if new governed docs are added.
...
- `rg -n 'target-project|canary-project|real-project|~/Dev/|\\.tilly/bin' docs scripts src`
  ...
  violation.
- `rg -n 'push|publish|marketplace|remote sync|write-capable' docs/roadmap/GOAL-SUPER-SPEC-cortex-hardening.md docs/mesh/CORTEX-MCP.md scripts`
...
```text
/goal
Materialize docs/roadmap/GOAL-SUPER-SPEC-cortex-hardening.md as the Cortex
hardening sequence for the three accepted findings, in incremental, auditable
execution with commit per material SPEC.

Use the Super SPEC as the complete execution contract. Do not duplicate or
compress its materialization tree in memory; preserve `SPEC-000` through
`SPEC-004` exactly and in order.

First mandatory act: run the preflight commands in `SPEC-000`, identify
unrelated pending changes, read the Super SPEC, and declare the file matrix
before editing.

Work mode: stage only the current SPEC, run focused oracle before broad oracle,
commit after each material SPEC, capture `git show --stat --oneline <commit>`,
post-commit status, reviewer result, negative checks, and sync status. Prior
commits are baseline-only unless the owner explicitly accepts them as execution
credit.

Stop on cross-project MCP leakage, unreconciled helper inventory, durable
untracked line undercount, private vocabulary leakage, required release
identity decision, or any unfixed focused oracle failure.

Final delivery must report each SPEC id, changed files, commit hash, focused
oracles, negative checks, reviewer result, post-commit status, sync status, and
any blocker. State explicitly that no remote, release, or target-project mirror
