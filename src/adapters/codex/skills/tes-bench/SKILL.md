...
Historical `docs/evidence/reports/context-mesh/<run-id>/` evidence remains
readable retained proof. It is not a current operational claim unless linked
from `docs/evidence/current/**` or another active claim document.
...
3. Expect new default runs under the temporal `reports/YYYY/MM/DD` path. Use an
   explicit `--out-root` only for temporary output or intentional legacy writes.
4. Use `npm run benchmark:converge` only after run artifacts exist, passing the
   exact `summary.json` path when reviewing a specific run.
5. Treat generated reports and raw traces as `source_of_truth: false`; use
   `docs/evidence/current/**` for current claim interpretation.
6. Preserve raw evidence and manifests when a claim depends on them.
7. State whether the result is planning evidence, fixture evidence, behavior
