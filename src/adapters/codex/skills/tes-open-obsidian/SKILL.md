...
`/tes-open-obsidian` is the preferred shared TES trigger for opening a TES-initialized project in Obsidian. `/tes:open-obsidian` is a compatible TES intent alias.
...
| Status | Meaning |
|---|---|
| `OPENED` | Obsidian launch command succeeded. |
| `READY` | Project context and alignment passed; open action was not executed or was dry-run only. |
| `BLOCKED` | Context, alignment, app detection, or launch failed. |
