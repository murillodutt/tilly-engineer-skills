...
`tes-upstream-first` is narrower than `tes-engineering-discipline`: it governs the
*approach* to a single class of decision (library friction / dependency change),
where `tes-engineering-discipline` is the broad engineering-discipline anchor. It
pairs with the global Context7 rule (step 1 of its checklist) and with `tes-bump`
when a dependency change implies a version decision.
