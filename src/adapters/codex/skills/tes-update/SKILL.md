...
`/tes-update` is the visible update entrypoint for an already installed TES mesh. `/tes:update` is a compatible intent alias; if a host rejects the colon form as slash text, continue as `/tes-update`.
