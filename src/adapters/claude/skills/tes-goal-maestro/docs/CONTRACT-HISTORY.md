...
| 2026-06-29 | Added optional Adversarial Audit Heartbeat Prompt generation: exact opt-in only, universal placeholders, read-only auditor authority, honest host-state boundary, separate heartbeat statuses, Cursor lazy coverage, and no stop-state ownership. | `references/adversarial-audit-heartbeat.md`; `templates/adversarial-audit-heartbeat.template.md`; `adversarial-audit-heartbeat-contract.mjs`; GM11 wall; Cursor runtime capability rule. | high |
| 2026-06-29 | Hardened installed-target feedback feature gates: GM10 validates delivered Goal Maestro skill surfaces when public docs are absent, GM11 resolves installed `.agents`/`.claude`/`.cursor` roots without requiring source `src/adapters`, and Cursor lazy coverage now includes Execution Thermometer without claiming skill parity. | `execution-thermometer-docs.mjs`; `execution-thermometer-integration.mjs`; `adversarial-audit-heartbeat-contract.mjs`; GM9C/GM10/GM11 walls; Cursor runtime capability rule. | high |
| 2026-06-29 | Hardened the Execution Thermometer static HTML report to an enterprise report layout: executive signal strip, anchor-selected loop detail pane, loop-scoped evidence/metrics, responsive tables, print expansion, and no JavaScript/runtime/network dependency. | `execution-thermometer-html.mjs`; GM4 wall; desktop/mobile rendered screenshots in P2 validation. | high |
