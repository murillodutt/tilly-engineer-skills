...
|----------|--------------|-------|
| OmniVoice | `scripts/tes_tts_omnivoice_provider.py status` then `speak` | Premium optional local cloned-voice provider when the maintainer has already configured its external Python environment and reference voice. |
