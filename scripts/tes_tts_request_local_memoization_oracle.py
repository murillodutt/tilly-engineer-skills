#!/usr/bin/env python3
"""Validate request-local memoization for TES TTS fast-path rendering."""
...
def render_once(case: dict[str, Any]) -> tuple[dict[str, Any], float]:
    ...
    result = render_request_local(case["source_text"], case["locale"])
...
def validate_shape(fixtures: dict[str, Any]) -> list[str]:
    ...
    if fixtures.get("usage") != "request_local_memoization_contract":
        failures.append("fixture usage must be request_local_memoization_contract")
    if fixtures.get("runtime_output") is not False:
        failures.append("memoization fixture must not claim runtime output")
    if fixtures.get("provider_timing") != "out_of_scope":
        failures.append("provider timing must remain out_of_scope")
    if fixtures.get("persistence") != "request_local_only":
        failures.append("memoization persistence must be request_local_only")
    cases = fixtures.get("cases")
    if not isinstance(cases, list) or len(cases) < 4:
        failures.append("memoization fixtures must include at least four cases")
        return failures
    required = {
        "id",
        "source_text",
...
def validate_case(case: dict[str, Any]) -> tuple[list[str], dict[str, Any]]:
    ...
    second_request, _ = render_once(case)
    if second_request["cache_misses"] != first_result["cache_misses"]:
        failures.append(f"{case['id']}: cache persisted across requests")
    if second_request["cache_hits"] != first_result["cache_hits"]:
        failures.append(f"{case['id']}: second request reused or skipped request-local cache")
    if first_result["memo_scope"] != "request_local":
        failures.append(f"{case['id']}: memo scope drifted")
    if first_result["spoken_text"] != case["expected_spoken_text"]:
    ...
    if first_result["resolved_once_keys"] != sorted(case["expected_resolved_once_keys"]):
        failures.append(f"{case['id']}: resolved-once keys drifted")
    if first_result["cache_hits"] != case["expected_cache_hits"]:
        failures.append(f"{case['id']}: cache hit count drifted")
    if first_result["cache_misses"] != case["expected_cache_misses"]:
        failures.append(f"{case['id']}: cache miss count drifted")
    ...
    if first_result["provider_timing"] != "out_of_scope":
        failures.append(f"{case['id']}: provider timing entered memoization output")
    if first_result["runtime_output"] is not False:
        failures.append(f"{case['id']}: memoization claimed runtime output")
    ...
    observed = {
        ...
        "resolved_once_keys": first_result["resolved_once_keys"],
        "second_request_cache_misses": second_request["cache_misses"],
        "text_prepare_ms_p50": round(median(timings), 3),
        "provider_timing": first_result["provider_timing"],
        "memo_scope": first_result["memo_scope"],
    }
...
def validate_fixtures(fixtures: dict[str, Any]) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    for case in fixtures["cases"]:
        ...
        proved_kinds.update(result["span_kinds"])
    required_kinds = {"protected_term", "path", "url", "code", "command", "exact_island", "secret"}
    missing = sorted(required_kinds - proved_kinds)
    if missing:
        failures.append(f"memoization fixtures did not prove span kinds: {missing}")
    return failures, observed
...
def main() -> int:
    ...
    print(f"[tes-tts-request-local-memoization] {status}")
