...
def surfaces(target: Path) -> dict[str, bool]:
    return {
        ...
        or (target / ".agents/skills/tilly-engineering-discipline/SKILL.md").exists(),
        "claude": (target / "CLAUDE.md").exists()
        or (target / ".claude-plugin/plugin.json").exists()
        or (target / "skills/tilly-guidelines/SKILL.md").exists(),
        ...
        "mcp_server": (target / ".tilly/bin/cortex_mcp.py").exists(),
        "field_reports": (target / ".tilly/bin/field_reports.py").exists()
...
def analyze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if state == "new":
        ...
        intent = f"/tilly:update {route}"
    else:
        intent = "/tilly:doctor" if update_status == "CURRENT" else f"/tilly:update {route}"
...
def self_test() -> dict[str, Any]:
    failures: list[str] = []
    with tempfile.TemporaryDirectory(prefix="tilly-update-") as tempdir:
        target = Path(tempdir)
        ...
        write(target / ".cursor/rules/tilly-guidelines.mdc", "Route to docs/agents/**\n")
        write(target / ".tilly/bin/cortex_mcp.py", 'VERSION = "0.3.24"\n')
        args = argparse.Namespace(
            target=target,
            remote_version="0.3.27",
            remote_commit="a" * 40,
            runtime="codex",
            offline=False,
            timeout=0.1,
        ...
        if result["recommended_intent"] != "/tilly:update all":
            failures.append("multi-runtime update intent must be /tilly:update all")

    with tempfile.TemporaryDirectory(prefix="tilly-update-single-") as tempdir:
        target = Path(tempdir)
        write(target / "docs/agents/PROJECT-REGISTER.md", "Generated by Tilly\n")
        write(target / "AGENTS.md", "Route to docs/agents/**\n")
        write(target / ".tilly/bin/tilly_update.py", f'VERSION = "{VERSION}"\n')
        args = argparse.Namespace(
            target=target,
    ...
    with tempfile.TemporaryDirectory(prefix="tilly-update-new-") as tempdir:
        target = Path(tempdir)
        args = argparse.Namespace(
            target=target,
            remote_version=VERSION,
        ...
        if result["recommended_intent"] != "/tilly:init":
            failures.append("new fixture must route to /tilly:init")
...
def main() -> int:
    ...
    print("[tilly-update] " + result["status"])
