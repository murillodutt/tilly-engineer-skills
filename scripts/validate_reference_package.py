...
def main() -> int:
    ...
    if package_json.exists():
        ...
        if package.get("version") != "0.2.4":
            failures.append("package.json version must be 0.2.4")
    ...
    for relpath in ("src/adapters/claude/plugin/plugin.json", "src/adapters/claude/plugin/marketplace.json"):
        path = ROOT / relpath
        if path.exists() and "0.2.4" not in path.read_text(encoding="utf-8"):
            failures.append(f"{relpath} must declare 0.2.4")
