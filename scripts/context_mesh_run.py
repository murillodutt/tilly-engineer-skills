...
SECTION_RULES = {
    "Mantra Gate": "Use the TES Mantra Gate for destructive, remote, release, sync, secret-bearing, or high-impact state changes, and for closure claims that depend on those actions. For ordinary local edits, focused oracles, staging, and local commits, keep the check inline and do not block on gate artifacts, markers, or skill loading. For risky actions, VERIFY evidence, SCOPE allowed territory, choose BEST_PATH, DOCUMENT the record when needed, name the ORACLE, RESOLVE ambiguity, and report STATUS before claiming progress.",
