...
def post_layer_zero_final_probe(result: dict[str, Any]) -> dict[str, Any]:
    # Ceiling F36: context_core_status=UNKNOWN means the core manifest entry /
    # hash is missing — core integrity is UNPROVEN, not proven-clean. UNKNOWN
    # must compose PENDING/NEEDS_EVIDENCE, never READY. Only PASS/NOT_APPLIED
    # (proven, or genuinely absent) admit readiness.
    ready = (
        result.get("helper_contract_status") == "PASS"
        and result.get("runtime_trigger_status") in {"PASS", "NOT_APPLIED"}
        and result.get("context_core_status") in {"PASS", "NOT_APPLIED"}
...
def _register_context_core(target: Path, relpath: str = "AGENTS.md") -> None:
    """Register a context-core bootloader in the install manifest with the
    contract's expected sha, so a genuinely-ready fixture PROVES its core
    (context_core_status PASS, not the unproven UNKNOWN). Ceiling F36: only a
    proven (or genuinely absent) core admits readiness — an unregistered core is
    correctly not READY. Merges into any existing manifest entries.
    """
    path = target / relpath
    core = None
    if root_context_helper is not None:
        try:
            core = root_context_helper.extract_core(read_text(path))
        except Exception:
            core = None
    sha = str((core or {}).get("sha256") or sha256_file(path))
    manifest_path = target / ".tes/manifest.json"
    manifest = read_json(manifest_path)
    entries = manifest.get("entries") if isinstance(manifest.get("entries"), list) else []
    entries = [e for e in entries if not (isinstance(e, dict) and e.get("path") == relpath and e.get("layer") == "context_governance")]
    entries.append({"layer": "context_governance", "path": relpath, "sha256": sha})
...
def mcp_config_adapters(route: str) -> list[str]:
    if route == "all":
...
def mcp_config_paths(route: str) -> list[str]:
    path_by_adapter = {
        "codex": ".codex/config.toml",
        "claude": ".mcp.json",
        "cursor": ".cursor/mcp.json",
    }
    return [path_by_adapter[adapter] for adapter in mcp_config_adapters(route)]


def mcp_config_refresh_commands(target: Path, route: str) -> list[str]:
    commands: list[str] = []
    for adapter in mcp_config_adapters(route):
        commands.append(
            f"python3 <tes-package>/scripts/install_mcp.py --target {target} --adapter {adapter} "
            "--dry-run --overwrite --json-only"
        )
        commands.append(
            f"python3 <tes-package>/scripts/install_mcp.py --target {target} --adapter {adapter} "
            "--overwrite --yes"
        )
    commands.append("python3 .tes/bin/tes_update.py plan --target . --json-only")
    return commands
...
def continuation_plan(
    ...
    phases = [
        ...
        },
        {
            ...
            "required": adapter_required,
            "approval_required": adapter_required,
            "writes": mcp_config_paths(route) if adapter_required else [],
            "commands": mcp_config_refresh_commands(target, route) if adapter_required else [],
            "goal": "refresh selected TES adapter MCP configs without implicitly adding VS Code consumer config",
        },
        {
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="tes-update-") as tempdir:
        ...
        # Ceiling F30 red-capable proof: on the SAME update-AVAILABLE fixture,
        # default `plan` exits 0 (query — drift is in the JSON) while --gate (and
        # the `status` command) exit non-zero, so a scripted caller's exit code
        # reflects that an update is needed instead of certifying success.
        import io as _io30
        import contextlib as _ctx30
        _orig_argv30 = sys.argv

        def _run_update_main(argv: list[str]) -> int:
            sys.argv = argv
            with _ctx30.redirect_stdout(_io30.StringIO()):
                return main()
        ...
        try:
            ...
            if _run_update_main(base30) != 0:
                failures.append("F30: default plan must exit 0 on an available update (query)")
            if _run_update_main(base30 + ["--gate"]) == 0:
                failures.append("F30: --gate must exit non-zero when an update is AVAILABLE")
            status30 = ["tes_update.py", "status", *base30[2:]]
            if _run_update_main(status30) == 0:
                failures.append("F30: `status` command must exit non-zero when an update is AVAILABLE")
        finally:
            sys.argv = _orig_argv30
    ...
    with tempfile.TemporaryDirectory(prefix="tes-update-single-") as tempdir:
        ...
        write(target / "AGENTS.md", trigger_fixture_text())
        _register_context_core(target)  # ceiling F36: prove the core so the ready fixture is READY
        write(target / ".agents/skills/tes-init/SKILL.md", trigger_fixture_text())
        write(target / ".agents/skills/tes-update/SKILL.md", trigger_fixture_text())
        write(target / ".agents/skills/tes-engineering-discipline/SKILL.md", trigger_fixture_text())
        ...
        # Red-capable Ceiling F36: context_core_status=UNKNOWN (core integrity
        # unproven) must NEVER compose READY. Same fully-green result but with
        # UNKNOWN core must fall to non-READY. Restoring UNKNOWN to the
        # readiness-admitting set turns this red.
        unknown_core_probe = post_layer_zero_final_probe({**result, "context_core_status": "UNKNOWN"})
        if unknown_core_probe.get("status") == "READY":
            failures.append("F36: context_core_status=UNKNOWN must not compose post-Layer Zero READY")
        proven_core_probe = post_layer_zero_final_probe({**result, "context_core_status": "PASS"})
        if proven_core_probe.get("status") != "READY":
            failures.append("F36: a proven (PASS) context core on an otherwise-ready target must be READY")
    ...
    with tempfile.TemporaryDirectory(prefix="tes-update-trigger-drift-") as tempdir:
        ...
        if "mcp_config_refresh" not in drift_phases:
            failures.append("runtime trigger drift continuation plan must require MCP config refresh")
        mcp_phase = next(
            (phase for phase in drift_plan.get("phases", []) if phase.get("name") == "mcp_config_refresh"),
            {},
        )
        if mcp_phase.get("writes") != [".mcp.json"]:
            failures.append("Claude runtime trigger drift must refresh only Claude MCP config")
        if any("--adapter all" in command or "vscode" in command for command in mcp_phase.get("commands", [])):
            failures.append("MCP config refresh must not route Claude drift through all or VS Code")
        if not any("--overwrite --yes" in command for command in mcp_phase.get("commands", [])):
            failures.append("stale tes-cortex MCP fixture must have an overwrite repair command")
    ...
    with tempfile.TemporaryDirectory(prefix="tes-update-field-report-") as tempdir:
        ...
        write(target / "docs/agents/PROJECT-REGISTER.md", "Generated by Tilly\n")
        write(target / "AGENTS.md", trigger_fixture_text())
        _register_context_core(target)  # ceiling F36: prove the core so this ready fixture is READY
        write(target / ".agents/skills/tes-init/SKILL.md", trigger_fixture_text())
        write(target / ".agents/skills/tes-update/SKILL.md", trigger_fixture_text())
        write(target / ".agents/skills/tes-engineering-discipline/SKILL.md", trigger_fixture_text())
        write(target / ".tes/bin/tes_update.py", helper_source_text("tes_update.py"))
...
def main() -> int:
    ...
    # Ceiling F30: query vs gate exit code. `plan` (default) is the query path —
    # it exits 0 and exposes update_status/update_available in the JSON, so a
    # drift never breaks a JSON-reading caller. --gate (or the `status` command)
    # opts a scripted caller into a non-zero exit when an update is AVAILABLE or
    # helpers/triggers/governance have drifted, so $?/`set -e` reflects the JSON.
    parser.add_argument("--gate", action="store_true", help="exit non-zero when an update is available or drift is detected")
    ...
    # Ceiling F30: in gate mode (--gate or the `status` command), the exit code
    # must reflect update readiness, not the always-PASS plan status. An
    # AVAILABLE update or any non-CURRENT update_status (drift / stale helpers /
    # unknown) exits non-zero so a scripted caller cannot read success while the
    # JSON reports an update is needed. The default `plan` path keeps exit 0.
    gate_mode = not args.self_test and (getattr(args, "gate", False) or args.command == "status")
    if gate_mode:
        update_status = str(result.get("update_status") or "")
        if result.get("update_available") or update_status not in {"CURRENT"}:
            if result["status"] == "BLOCKED":
                return 2
            return 1
