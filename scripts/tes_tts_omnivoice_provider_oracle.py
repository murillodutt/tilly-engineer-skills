...
def validate_server_routes_removed() -> list[str]:
    failures: list[str] = []
    source = PROVIDER_SCRIPT.read_text(encoding="utf-8")
    removed_tokens = (
        "server" + "-status",
        "speak" + "-server",
        "speak-long" + "-server",
        "legacy_lab" + "_compatibility",
    )
    for token in removed_tokens:
        if token in source:
            failures.append(f"obsolete execution token remains in provider script: {token}")
    help_completed = subprocess.run(
        [sys.executable, str(PROVIDER_SCRIPT), "--help"],
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=False,
    ...
    if help_completed.returncode != 0:
        failures.append(f"provider help returned unexpected exit code {help_completed.returncode}")
        return failures
    for token in removed_tokens[:3]:
        if token in help_completed.stdout:
            failures.append(f"obsolete server command remains in CLI help: {token}")
    return failures
...
def validate_long_read_dry_run_payload(payload: dict[str, Any] | None) -> list[str]:
    ...
    if payload.get("text_mode") != "redacted_source":
        failures.append("speak-long dry-run must keep redacted_source as the default provider surface")
...
def validate_sentence_aware_chunking_contract() -> list[str]:
    ...
    decimal_text = "A versão 0.3.150 mantém 9.2 como nota humana. Depois, o benchmark continua."
...
def validate_human_rated_quality_recipe_contract() -> list[str]:
    ...
    each_chunk_args = argparse.Namespace(prosody_warmup="confirmation-en", prosody_warmup_scope="each_chunk")
    if [provider_module.prosody_warmup_for_chunk(each_chunk_args, index) for index in range(1, 4)] != [
        "confirmation-en",
        "confirmation-en",
        "confirmation-en",
    ]:
...
def main() -> int:
    ...
    server_failures = validate_server_routes_removed()
    failures.extend(server_failures)
    add_partition(
        partitions,
        name="server_lab_purge",
        focus="obsolete server and lab routes are absent from the product provider surface",
