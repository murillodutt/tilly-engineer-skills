...
| 2026-07-02 | `tes.host_transcript_canary@0.1.8`: Added an agent-hook finding manifest, `agent_hook_finding_matrix.py`, and a finding report template so aggregate hook certification cannot be mistaken for per-finding certification. | Maintainer request after the Claude host ceiling closeout exposed the aggregate-vs-finding evidence gap. | high |
