...
Use external skill-design input as packaging guidance, not as a new authority
layer.
...
4. Add scripts only for deterministic validation, formatting, or repeatable
   error handling.
...
Apply packaging work only after behavior is clear. Packaging cannot rescue an
unclear workflow.

## Do Not Promote

Do not turn packaging guidance into public product surface, installer docs,
package manifests, or commercial claims until repeated local evidence proves it
improves precision without adding contextual noise.
